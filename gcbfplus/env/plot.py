...
def render_video(
    ...
    waypoints = graph0.waypoints
    waypoint_circs = [plt.Circle(waypoints[ii], r, color=goal_color, linewidth=0.0) for ii in range(n_agent)]
    waypoint_col = MutablePatchCollection([i for i in reversed(waypoint_circs)], match_original=True, zorder=6)
    ax.add_collection(waypoint_col)
    ...
    def init_fn() -> list[plt.Artist]:
        return [agent_col, waypoint_col, edge_col, *agent_labels, cost_text, *safe_text, *cnt_col, kk_text]
    ...
    def update(kk: int) -> list[plt.Artist]:
        ...
        waypoints_t = graph.waypoints
        for ii in range(n_agent):
            waypoint_circs[ii].set_center(tuple(waypoints_t[ii]))
        waypoint_col.set_offsets(waypoints_t[:2])
        ...
        return [agent_col, waypoint_col, edge_col, *agent_labels, cost_text, *safe_text, *cnt_col_t, kk_text]
