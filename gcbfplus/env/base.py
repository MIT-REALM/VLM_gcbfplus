...
class MultiAgentEnv(ABC):
    ...
    @abstractmethod
    def multi_leader_follower_assign(self, graph: GraphsTuple, leader: Array, goal: Array = None) -> GraphsTuple:
        pass
    ...
    def best_leader_graph(self, graph: GraphsTuple, actor: Callable) -> GraphsTuple:
        ...
        """
        leaders = jnp.arange(self.num_agents)  
        goal_pos_rel = jnp.array([[0.0, 1.0], [0.0, -1.0], [1.0, 0.0], [-1.0, 0.0]])  
        agent_pos = graph.type_states(type_idx=0, n_type=self.num_agents)[:, :2]
        ...
        goal_pos = jax.vmap(get_goal_pos)(agent_pos)  
        ...
        leader_graphs = jax.vmap(assign_one_leader)(leaders, goal_pos)  

        
        actions = jax.vmap(jax.vmap(actor))(leader_graphs)  
        actions_mean = jnp.linalg.norm(actions, axis=-1).mean(axis=-1)  
        
        

        
        goal_dir = ((real_goal_pos - agent_pos) /
                    (jnp.linalg.norm(real_goal_pos - agent_pos, axis=-1, keepdims=True) + 1e-6))  
        average_goal_dir = jnp.mean(goal_dir, axis=0)[None, None, :]   
        actions_mean_dir = jnp.mean(actions, axis=2)    
        dot_prod = jnp.sum(average_goal_dir * actions_mean_dir, axis=-1)  
    ...
    def rollout_fn_jitstep(
        ...
        jit_mulit_leader_follower_assign  = jax.jit(self.multi_leader_follower_assign)
        ...
        def fn(key: PRNGKey) -> [RolloutResult, Array, Array]:
            ...
            all_obs = []
            jit_unsafe_mask_fn = jax.vmap(jax.jit(self.unsafe_obs_mask))
            jit_envstate = jax.jit(self.EnvState)
            jit_get_graph = jax.jit(self.get_graph)
            total_leader_time = 0
            leader_call_time = 0
            for kk in tqdm.trange(rollout_length, ncols=80):                
                
                if self._use_leader:
                    graph, num_LLM_calls, prompt, all_prompts, leader_assign_count, t_mode, prompt_time_gap, sent_token_count, received_token_count, leader_id, leader_call_time = llm_utils.leader_graph(graph, jit_policy, jit_leader_follower_assign, kk, self.leader_control, policy,prompt,all_prompts,self.reset_graph, leader_assign_count, t_mode, prompt_time_gap, num_LLM_calls, sent_token_count, received_token_count, leader_id, jit_unsafe_mask_fn, jit_envstate,jit_get_graph,jit_mulit_leader_follower_assign)

                total_leader_time += leader_call_time
                ...
                if kk > 10 and avg_speed< 0.001 and leader_model is None:
                    break
            ...
            unsafe_masks = jit_unsafe_mask_fn(graphs_for_points)
            ...
            stat_output = mean_init_distance, final_mean_distance, num_LLM_calls, dist_traveled, kk, sent_token_count, received_token_count, finish_count, finish, avg_speed_last_100, total_leader_time
            with open(f"{log_dir}/log_message.txt", "a") as f:
                ...
                f.write('\n')
                f.write('Total leader time: ')
                f.write(str(total_leader_time))
                f.write('\n')
